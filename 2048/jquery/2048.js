  ...
  updateView: function () {
    for (var r = 0; r < this.RN; r++) {
      for (var c = 0; c < this.CN; c++) {
        var div = $('#c' + r + c);
        if (this.data[r][c] !== 0) {
          div.html(this.data[r][c]);
          div[0].className = 'cell n' + this.data[r][c];
        } else {
          div.html('');
          div[0].className = 'cell';
        }
      }
    }
    $('#score').html(this.score);
    $('#gameOver').css({'display': (this.state === this.GAMEOVER ? 'block' : 'none')});
    this.state === this.GAMEOVER && ($('#final').html(this.score));
  },
  randomNum: function () {
    while (true) {
